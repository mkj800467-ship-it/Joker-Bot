// plugins/game-eye.js
// ✧ UCHIHA - Uchiha Itachi - لعبة تخمين الشخصية من العين 👁️
...
let handler = async (m, { conn, command, usedPrefix }) => {
    ...
    if (command.startsWith("مجوب_")) {
        ...
        if (isCorrect) {
            ...
            await conn.reply(m.chat, theme.build([
                ...
                { type: 'line', text: `🎉 أحسنت أيها المحارب! لقد وقعت في الوهم واستحقرت البصيرة +${rewardPoints} نقطة و +${rewardCoins} ذهب` },
                { type: 'divider' },
            ...
            if (obito.attempts > 0) {
                ...
                await conn.reply(m.chat, theme.build([
                    ...
                    { type: 'divider' },
                    { type: 'line', text: '👁️ شاركوني الشارينگان وحاول مرة أخرى' }
                ]), m);
            } else {
                await m.react("❌");
                await conn.reply(m.chat, theme.build([
    ...
    try {
        ...
        const interactiveMessage = {
            body: {
                text: theme.build([
                    { type: 'title', text: '👁️ إتاتشي: "مهمة تخمين الشخصية"' },
                    { type: 'spacer' },
                    { type: 'line', text: '🔮 *من صاحب هذه العين الشارينگان؟*' },
                    { type: 'divider' },
            ...
            footer: { text: '⛩️ Uchiha Itachi - Sharingan Eye ⛩️' },
            header: {
