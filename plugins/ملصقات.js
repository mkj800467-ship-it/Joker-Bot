...
// ✧ Itachi & The Joker - حزمة ملصقات Pinterest 📦
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!text) {
    return m.reply(`*الاستخدام:* ${usedPrefix + command} <كلمة البحث> <العدد>\n*مثال:* ${usedPrefix + command} anime 10`)
  }
  ...
  let statusMsg = await m.reply(`⏳ جاري البحث عن "${query}"... أرجو الانتظار قليلاً`)
  ...
  try {
    ...
    if (allStatic.length === 0 && allAnimated.length === 0) {
      return m.reply(`❌ عذراً، لا توجد نتائج مطابقة لـ "${query}"`)
    }
    ...
    if (pack.length === 0) {
      return m.reply(`⚠️ تعذر معالجة الملصقات المطلوبة`)
    }
    ...
    console.error(e)
    try { await conn.sendMessage(m.chat, { delete: statusMsg.key }) } catch {}
    m.reply(`⚠️ حدث خطأ أثناء إنشاء الحزمة: ${e.message}`)
  }
}
